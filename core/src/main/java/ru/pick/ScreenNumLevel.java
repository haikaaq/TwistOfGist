...
public class ScreenNumLevel implements Screen {
    ...
    public ScreenNumLevel(Main main){
        ...
        btnLevel= new SpaceButton(font, "level "+main.level, 1350);
    ...
    public void render(float delta) {
        ...
        btnNumLevel.changeText(curentlevel.numLevel);
        btnLevel.changeText("level "+level);
        batch.setProjectionMatrix(camera.combined);

        batch.begin();
        batch.draw(imgBG, 0, 0, SCR_WIDTH, SCR_HEIGHT);
        btnNumLevel.font.draw(batch,btnNumLevel.text,btnNumLevel.x,btnNumLevel.y);
